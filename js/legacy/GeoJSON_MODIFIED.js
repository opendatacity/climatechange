    ...
    function buildGeom(item, params) {
        ...
        for(var gtype in params.geom) {
            ...
            // Geometry parameter specified as: {Point: ['lat', 'lng']}
                ...
                //TODO: MY EDIT: only allows for a 4 point polygon to be created and assumes a single point to add and subtract from.
            // Geometry parameter specified as: {Polygon: [['lat', 'lng'], ['lat', 'lng'], ['lat', 'lng'], ['lat', 'lng']]}
            else if(Array.isArray(val) && item.hasOwnProperty(val[0][0]) && item.hasOwnProperty(val[1][1]) && item.hasOwnProperty(val[2][0]) && item.hasOwnProperty(val[3][1])){
                geom.type = gtype;
                geom.coordinates = [[[item[val[0][1]] - 0.125, item[val[0][0]] - 0.125], [item[val[1][1]] + 0.125, item[val[1][0]] - 0.125], [item[val[2][1]] + 0.125, item[val[2][0]] + 0.125], [item[val[3][1]] - 0.125, item[val[3][0]] + 0.125]]];
            }
        }
